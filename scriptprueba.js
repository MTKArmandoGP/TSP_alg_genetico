...
function getCoordinates() {
  let markers = markersLayer.getLayers();
  coordinatesArray = markers.map((marker) => marker.getLatLng());
  let coordinates = coordinatesArray.map(
    (coordinate) => `Latitud: ${coordinate.lat}, Longitud: ${coordinate.lng}`
  );
  alert(`Coordenadas:\n${coordinates.join("\n")}`);
}
...
function showPlot() {
  let markers = markersLayer.getLayers();
  let xValues = markers.map((marker) => marker.getLatLng().lat);
  let yValues = markers.map((marker) => marker.getLatLng().lng);
  ...
  let table = "<table><tr><th>Latitud</th><th>Longitud</th></tr>";
  for (let i = 0; i < coordinates.length; i++) {
    table += `<tr><td>${coordinates[i][0]}</td><td>${coordinates[i][1]}</td></tr>`;
  }
  table += "</table>";
  document.getElementById("coordinates-table").innerHTML = table;
}
function showDistance() {
  ...
  // Convert to kilometers and round to two decimal places
...
function alg_Genetico() {
  ...
  function crossover(parent1, parent2) {
    ...
    if (randomNumber < 0.2) {
      const cutIndex = Math.floor(Math.random() * parent1.length);
  ...
  for (let generation = 0; generation < maxGenerations; generation++) {
    ...
    while (newPopulation.length < populationSize) {
      ...
      const fitnessValues = individuals.map(fitness);
